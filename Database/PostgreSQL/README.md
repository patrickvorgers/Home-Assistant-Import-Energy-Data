...
<h1 align="center">How-to PostgreSQL</h1>

  <p align="center">
How-to for importing historical energy/water data from external datasources into the Home Assistant PostgreSQL database so that it can be used in the Energy Dashboard.
    <br />
    <br />
  </p>
</div>
...
#### Tooling
- Download and install: DBeaver https://dbeaver.io/ or pgAdmin https://www.pgadmin.org/
...
- Start `DBeaver` or `pgAdmin`
- On first run configure the database connection
  - Database type: PostgreSQL
  - Hostname/IP: hostname/ip-number of your PostgreSQL machine
    - In case PostgreSQL runs on the same machine as Home Assistant this is the same hostname/ip-number of your Home Assistant instance
  - User/password: database user and password
    - The database user and password can be found in the connection string in the `configuration.yaml`
      `
      recorder:
          db_url: postgresql://<user>:<password>@core-postgresql/homeassistant
      `
    - Port: 5432 (default)
...
- Run the `ImportData.py` script from the Datasources directory to import the generated CSV files and use the following command-line parameters:
    - `--db-type postgresql`
    - `--host HOST` Host as defined in the database connection
    - `--user USER` Username as defined in the database connection
    - `--password PASSWORD` Password as defined in the database connection
    ...
    - `--verbose`<br><br>
    Example:<br>
`python ImportData.py --db-type postgresql --host localhost --user postgres --database homeassistant --csv-file "data\*.csv" --verbose`
- Verify by querying the database that it now contains the table `import_data`
...
- Run the `Sensors.py` script from the Database directory to determine the sensor configuration and use the following command-line parameters:
    ...
    Example:<br>
`python Sensors.py --db-type postgresql --host localhost --user postgres --database homeassistant`
